...
async function supaGetPostsTable() {
    const url = `${supaUrl}/rest/v1/${POSTS_TABLE}?select=*&order=created_at.desc`;
    const r = await fetch(url, { headers: supaHeaders() });
    if (!r.ok) {
        const text = await r.text();
        throw new Error(`Supabase posts GET ${r.status}: ${text.slice(0, 200)}`);
    }
    const rows = await r.json();
    return Array.isArray(rows) ? rows.map(normalizePostRow) : [];
}
...
function normalizePostRow(row) {
    ...
    if (!Array.isArray(post.related)) post.related = [];
...
async function getPosts() {
    if (useSupabase) {
        try {
            const rows = await supaGetPostsTable();
            if (rows.length) return rows;
        } catch (e) {
            console.error('[store] supabase posts table get error:', e.message);
        }
    }
...
/**
 * Basic XSS protection by escaping HTML special characters.
 */
...
module.exports = { getJSON, setJSON, getPosts, getMessages, isWritable, envInfo, escapeHTML, sanitizeInput };
