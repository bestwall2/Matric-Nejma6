...
async function handleAPI(req, res, parsed) {
    ...
    if (pathname === '/api/messages' && method === 'POST') {
        ...
        if (!isValidEmail(email)) return sendError(res, 400, 'البريد الإلكتروني غير صحيح');
        if (!subject) return sendError(res, 400, 'الموضوع مطلوب');
        if (!message || message.length < 5) return sendError(res, 400, 'الرسالة قصيرة جداً');
    ...
    if (pathname === '/api/admin/login' && method === 'POST') {
        ...
        if (!rateLimitLogin(ip)) return sendError(res, 429, 'محاولات كثيرة، حاول لاحقاً.');
    ...
    if (pathname === '/api/admin/stats' && method === 'GET') {
        ...
        const recent = messages.filter(m => new Date(m.createdAt).getTime() >= last7).length;
    ...
    if (pathname === '/api/admin/posts' && method === 'POST') {
        let body; try { body = await readBody(req, 1024 * 200); } catch (e) { return sendError(res, 400, e.message); }
        const posts = readJSON(POSTS_FILE, []);
        const slug = safeStr(body.slug, 120);
        if (!slug) return sendError(res, 400, 'slug مطلوب');
        if (posts.some(p => p.slug === slug)) return sendError(res, 409, 'يوجد مقال بنفس الـ slug');
    ...
    if (pathname === '/api/admin/password' && method === 'POST') {
        ...
        if (next.length < 6) return sendError(res, 400, 'كلمة المرور الجديدة قصيرة (6 حروف على الأقل)');
        const admin = readJSON(ADMIN_FILE, {});
        if (!verifyPassword(current, admin.passwordHash)) return sendError(res, 401, 'كلمة المرور الحالية خاطئة');
        writeJSON(ADMIN_FILE, { passwordHash: hashPassword(next), updatedAt: new Date().toISOString() });
        return sendJSON(res, 200, { ok: true });
    }
